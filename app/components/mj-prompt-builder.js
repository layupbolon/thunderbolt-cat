export function IframeShare(config) {
...
<div class="IframeShare-embed IframeShare-${
  ...
  >
    ${config.buttonText || ''}
    <span class="IframeShare-button-icon IframeShare-close-icon">${svgIcon.close}</span>
...
`;
  ...
  function allowRepeatSubmit() {
    ...
    if (localStorage.getItem('submit') === 'true' && !config.allowRepeatSubmit && a) {
      a.style.opacity = '0';
      const t = setTimeout(() => {
  ...
  function submitAfterClose() {
    const closeBtn = dom('IframeShare-launch-button') || dom('IframeShare-close-icon');
